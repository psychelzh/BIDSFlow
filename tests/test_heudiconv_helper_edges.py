...
def test_low_level_run_helpers_cover_status_paths(tmp_path: Path, monkeypatch) -> None:
    ...
    assert not h._unit_status_is_succeeded(unit.status_path)
    h._write_unit_status(
        unit,
        status="succeeded",
        backend="sge",
    ...
    assert h._unit_status_is_succeeded(unit.status_path)
    ...
    h._release_unit_claim(unit)
    h._write_unit_claim(unit=unit)
    ...
    h._write_unit_claim(unit=other_unit)
    h._release_unfinished_claims((unit, other_unit), [], unit)
    assert not unit.claim_path.exists()
    assert not other_unit.claim_path.exists()

    unit.status_path.write_text("# comment\nignored\nstatus=succeeded\n", encoding="utf-8")
    assert h._read_key_value_status(unit.status_path) == {"status": "succeeded"}
...
def test_low_level_path_and_scheduler_helpers(tmp_path: Path) -> None:
    ...
    assert not h._cleanup_run_execution_view(project_root, outside_path)
    with pytest.raises(ValueError, match="outside the project root"):
        h._remove_project_path(project_root, outside_path)
    with pytest.raises(h.HeudiconvRunError, match="must name an immediate child directory"):
        h._resolve_sources_source_path(project_root, "../SUB001")

    log_path = project_root / "logs" / "unit.log"
    h._append_log(log_path, "first")
    h._append_log(log_path, "second")
    assert log_path.read_text(encoding="utf-8") == "first\n\nsecond\n"

    assert h._combine_process_output("out\n", "err\n") == "out\nerr"
    assert h._parse_sge_job_id(None) == ""
    assert h._parse_sge_job_id("123.1 first line\nignored") == "123.1 first line"
    ...
    with pytest.raises(h.SourcesError, match="Existing BIDSFlow sources state"):
        h._guard_sources_reset_requirement(sources_plan, reset=False)
    ...
    with pytest.raises(h.SourcesError, match="outside the project root"):
        h._reset_sources_state(project_root, outside_sources_plan)
